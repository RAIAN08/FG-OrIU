...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None, prepare_feature=False, training=False):
        features = {}
        ...
        if self.up:
            for i, (attn, ff) in enumerate(self.layers):
                ...
                if not training:
                    features[f"layer_{i}_attn"] = x.detach().cpu()  
                else:
                    features[f"layer_{i}_attn"] = x
                
                x = ff(x)
                
                if not training:
                    features[f"layer_{i}_ff"] = x.detach().cpu()
                else:
                    features[f"layer_{i}_ff"] = x
...
class ViT_face(nn.Module):
    ...
    def forward(self, img, label=None, mask=None, prepare_feature=False, training=False):
        """
        ...
        if prepare_feature:
            x, features = self.transformer(x, mask, prepare_feature=prepare_feature, training=training)
        else:
            x, _ = self.transformer(x, mask, prepare_feature=prepare_feature, training=training)
            features = None
